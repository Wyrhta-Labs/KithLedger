...
- **MCP surface:** `src/mcp/` assembles the `kith.*` tool registry via core's `createMcpServer`; tools call the service layer directly and share REST's auth and audit trail. The MCP server reads a `kl_` API key from the `KITHLEDGER_MCP_API_KEY` environment variable (create one via `POST /api/v1/auth/keys`, JWT-authenticated) — a valid key resolves to the admin user, and a missing/invalid/non-`kl_` value is rejected on the first tool call. As of B8 the adapter also rejects a key whose `kind` is not `member`: `McpPrincipal` has no room for the credential kind and the tools resolve `memberScope(ctx.principal.userId)`, so a household or ops key here would silently widen to its issuer's personal scope. Run with `npm run mcp`.
